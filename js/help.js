...
export const HELP = [
  ...
  { group: 'The tabs', topics: [
    ...
    { id: 'lab', title: 'Pick Lab', keys: 'pick lab insights style consensus model race simulation what-if what if win odds chance league versus', body: `
      ...
      <p><b>Why "usual" and not "hot streak"?</b> Across three seasons of this pool (2023–2025), how someone <i>picks</i> barely changes from year to year (underdog share correlates 0.85–0.87 from one season to the next), but how they <i>scored</i> doesn't carry over at all (weekly average 0.02–0.04, cover rate 0.04–0.09, 10s about 0). So the Lab treats style as real and past results as trivia, and the Commentator does too.</p>
    ...
    { id: 'history', title: 'History (past seasons)', keys: 'history 2024 2025 last year past seasons archive champions week by week bowls lessons all seasons year over year held up flipped', body: `
      <p>The <b>History</b> tab rebuilds past seasons pick by pick from the commissioner's files, with every game graded against the final score. Seasons on file: 2025 and 2024, complete; about 98–99% of rebuilt cards match the official weekly scores exactly, and the official scores always win. <b>2023 is partial</b>: the files stop after week 18, so week 19 and the final result are missing; only some weeks have official scores (the rest are rebuilt and checked against the season totals); and some bowl results come from ESPN rather than the commissioner. Everything from 2023 is marked with an asterisk (*) and a note, and the Commentator says so when it uses 2023.)</p>
    ...
    { id: 'scouting', title: 'Scouting reports (tap any name)', keys: 'scouting report profile player name tap analysis trends quirks neighborhood strategy', body: `
      ...
      <p>Pick habits pool every season on file (about 520 picks per person across 2023–2025). The week-by-week chart has a button for each season. Useful for thinking about strategy, not guarantees.</p>` },
